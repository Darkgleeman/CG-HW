...
void Shader::initWithCode(const std::string &vs, const std::string &fs, const std::string& gs) {
    GLuint vertexShader, fragmentShader, geoShader;
  ...
  if (gs != "")
  {
      ...
      const GLchar* gsCode = gs.c_str();
      glShaderSource(geoShader, 1, &gsCode, nullptr);
      glCompileShader(geoShader);
      glGetShaderiv(geoShader, GL_COMPILE_STATUS, &success);
      if (!success) {
          glGetShaderInfoLog(geoShader, sizeof(infoLog), nullptr, infoLog);
          LOG_ERR(infoLog);
      }
  }
